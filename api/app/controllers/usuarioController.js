...
function index(req, res) { //Lee todos
    Usuario.find({})
        .then(usuarios => {
...
function show(req, res) { //Lee uno
    if (req.body.error) return res.status(500).send({ error });
    if (req.body.usuarios) return res.status(200).send({ usuarios });
    return res.status(404).send({ message: 'NOT FOUND' });
}

function create(req, res) { //Crea uno nuevo
    let usuario = new Usuario(req.body);
    usuario.save().then(usuario => res.status(201).send({ usuario })).catch(error => res.status(500).send({ error }));
}

function update(req, res) { //Acualiza uno
    if (req.body.error) return res.status(500).send({ error });
...
function remove(req, res) { //Elimina uno
    if (req.body.error) return res.status(500).send({ error });
    if (!req.body.usuarios) return res.status(404).send({ message: 'NOT FOUND' });
    req.body.products[0].remove().then(usuario => res.status(200).send({ message: 'REMOVED', usuario })).catch(error => res.status(500).send({ error }));
}
